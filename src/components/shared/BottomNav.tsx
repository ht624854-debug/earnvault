...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-ev-bg/95 backdrop-blur-xl border-t border-ev-card-border safe-area-bottom">
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-ev-blue rounded-full"
                  transition={{ type: 'spring', stiffness: 350, damping: 30 }}
                />
              )}
              <item.icon
                className={`w-5 h-5 transition-colors ${
                  isActive ? 'text-ev-blue' : 'text-[#94A3B8]'
                }`}
              />
              <span

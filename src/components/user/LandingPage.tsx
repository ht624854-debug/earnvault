...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F7FF]">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-[#F0F7FF]/80 backdrop-blur-xl border-b border-[#EFF6FF]">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 ev-gradient-red rounded-lg flex items-center justify-center">
              <DollarSign className="w-5 h-5 text-white" />
            </div>
            <span className="text-xl font-bold text-[#1E293B]">{brandName}</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate('login')}
              className="text-sm text-[#64748B] hover:text-[#1E293B] transition-colors px-3 py-2"
            >
              Login
            </button>
            <button
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute top-20 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[#2563EB]/5 rounded-full blur-[120px]" />
        <motion.div
        ...
        >
          <motion.div variants={fadeInUp} className="mb-6">
            <span className="inline-flex items-center gap-2 bg-[#FFFFFF] border border-[#EFF6FF] rounded-full px-4 py-1.5 text-sm text-[#2563EB]">
          ...
          <motion.h1
            variants={fadeInUp}
            className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-[#1E293B] leading-tight mb-6"
          >
            {headline}
          </motion.h1>
          <motion.p
            variants={fadeInUp}
            className="text-[#64748B] text-base sm:text-lg max-w-2xl mx-auto mb-10"
          >
            {subheadline}
          </motion.p>
      ...
      {/* Stats */}
      <section className="py-12 border-y border-[#EFF6FF]">
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-4">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-2xl sm:text-3xl font-bold text-[#1E293B] mb-3">Why Choose {brandName}?</h2>
            <p className="text-[#64748B]">Everything you need to start earning online</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((feature, i) => (
              <motion.div
                key={i}
                className="ev-card p-6 hover:border-[#2563EB]/30 transition-colors group"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
        ...
        <section className="py-20 bg-[#F0F7FF]">
          <div className="max-w-6xl mx-auto px-4">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-2xl sm:text-3xl font-bold text-[#1E293B] mb-3">What Our Members Say</h2>
              <p className="text-[#64748B]">Real stories from real earners</p>
            </motion.div>
              ...
              {testimonials.map((t, i) => (
                ...
                >
                  <div className="flex items-center gap-1 mb-3">
                    {[...Array(5)].map((_, si) => (
                      <Star key={si} className="w-4 h-4 text-[#2563EB] fill-[#2563EB]" />
                    ))}
                  </div>
                  <p className="text-[#64748B] text-sm mb-4">&ldquo;{t.text}&rdquo;</p>
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-4">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-2xl sm:text-3xl font-bold text-[#1E293B] mb-3">Supported Payment Methods</h2>
            <p className="text-[#64748B]">Withdraw your earnings with ease</p>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {paymentMethods.map((name, i) => (
              <motion.div
                key={i}
                className="ev-card p-5 text-center hover:border-[#2563EB]/30 transition-colors"
              ...
              >
                ...
                <p className="text-[#1E293B] font-medium text-sm">{name}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          >
            ...
            <div className="relative z-10">
              <h2 className="text-2xl sm:text-3xl font-bold text-[#1E293B] mb-4">Ready to Start Earning?</h2>
              <p className="text-[#64748B] mb-4 max-w-lg mx-auto">
                Join {brandName} today and start earning real money by completing simple tasks. Registration is free!
              </p>
              <div className="mb-8 flex items-center justify-center gap-2">
                <span className="text-[#64748B] text-sm">Activation Fee:</span>
                {hasOffer ? (
                  <>
                    <span className="text-[#64748B] line-through text-sm">Rs. {activationFee}</span>
                  ...
                  <span className="text-[#1E293B] font-bold text-lg">Rs. {activationFee}</span>
                )}
              </div>
              <button
                onClick={() => navigate('register')}
      ...
      {/* Footer */}
      <footer className="border-t border-[#EFF6FF] py-10">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold text-[#1E293B]">{brandName}</span>
            </div>
              ...
              <div className="flex items-center gap-3">
                {socialLinks.map((s, i) => (
                  <a
                    ...
                    className="w-10 h-10 bg-[#FFFFFF] border border-[#EFF6FF] rounded-lg flex items-center justify-center text-[#64748B] hover:text-[#2563EB] hover:border-[#2563EB]/30 transition-all"
                    aria-label={s.label}
                  >

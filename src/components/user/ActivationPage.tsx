    ...
    <div className="min-h-screen bg-[#0A0A0A] pb-24">
      ...
      <div className="bg-[#0A0A0A] border-b border-[#1F1F1F] sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <h1 className="text-lg font-bold text-[#F5F5F5]">Account Activation</h1>
          <p className="text-xs text-[#737373] mt-0.5">Activate your account to unlock earning features</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 space-y-5">
            ...
            <div className="space-y-2.5">
              {benefits.map((b, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <b.icon className="w-4 h-4 text-[#DC2626]" />
                  <span className="text-sm text-[#737373]">{b.text}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {!isActive && (
          <motion.div className="ev-card p-5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <h2 className="text-base font-semibold text-[#F5F5F5] mb-4">Submit Activation Request</h2>
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              {form.payment_method_id && (() => {
                ...
                if (selectedPm?.instructions) {
                  return (
                    <div className="bg-[#1A1A1A] border border-[#262626] rounded-lg p-3 text-xs text-[#737373]">
                      {selectedPm.instructions}
                      {selectedPm.account_number && (
                        <p className="mt-1 text-[#F5F5F5]">Account: {selectedPm.account_number}</p>
                      )}
                    </div>
                  );
                }
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[#737373] mb-1.5">Sender Name</label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-medium text-[#737373] mb-1.5">Sender Number</label>
                  <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[#737373] mb-1.5">Transaction ID</label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-medium text-[#737373] mb-1.5">Amount</label>
                  <input
                    type="number"
                    value={form.amount}
              ...
              <div>
                <label className="block text-sm font-medium text-[#737373] mb-1.5">Proof Image (Optional)</label>
                {proofPreview && (
                  <div className="relative mb-2">
                    <img src={proofPreview} alt="Proof" className="w-full h-32 object-cover rounded-lg border border-[#1F1F1F]" />
                    <button
        ...
        {/* Existing Requests */}
        <div>
          <h2 className="text-base font-semibold text-[#F5F5F5] mb-3">Your Requests</h2>
              ...
              {existingRequests.map((req) => (
                <div key={req.id} className="ev-card p-4">
                  ...
                  <div className="grid grid-cols-2 gap-y-1 text-xs text-[#737373]">
                    <span>Amount:</span>
                    <span className="text-[#F5F5F5]">Rs. {req.amount}</span>
                    <span>TXN ID:</span>
                    <span className="text-[#F5F5F5]">{req.transaction_id}</span>
                    <span>Date:</span>

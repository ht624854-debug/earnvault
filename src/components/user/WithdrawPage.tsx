      ...
      <div className="min-h-screen bg-ev-bg pb-24">
        <div className="bg-ev-bg border-b border-ev-card-border sticky top-0 z-40">
          <div className="max-w-4xl mx-auto px-4 py-4">
            <h1 className="text-lg font-bold text-ev-text">Withdraw</h1>
          </div>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-16 text-center">
          <AlertTriangle className="w-14 h-14 text-[#F59E0B] mx-auto mb-4" />
          <h2 className="text-lg font-semibold text-ev-text mb-2">Account Not Active</h2>
          <p className="text-sm text-ev-muted mb-6">Activate your account to withdraw earnings</p>
  ...
  return (
    <div className="min-h-screen bg-ev-bg pb-24">
      ...
      <div className="bg-ev-bg border-b border-ev-card-border sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <h1 className="text-lg font-bold text-ev-text">Withdraw</h1>
          <p className="text-xs text-ev-muted mt-0.5">Withdraw your earnings</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 space-y-5">
        ...
        <motion.div className="ev-card p-5 relative overflow-hidden" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-ev-muted">Available Balance</p>
              <p className="text-2xl font-bold text-ev-text">Rs. {balance.toLocaleString()}</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="ev-card p-5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-base font-semibold text-ev-text mb-4">Request Withdrawal</h2>
          <p className="text-xs text-ev-muted mb-4">
            Minimum withdrawal: <span className="text-ev-blue font-semibold">Rs. {minWithdraw}</span>
          </p>
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <div>
              <label className="block text-sm font-medium text-ev-muted mb-1.5">Amount (Rs.)</label>
              <input
                type="number"
                value={form.amount}
            ...
            <div>
              <label className="block text-sm font-medium text-ev-muted mb-1.5">Account Holder Name</label>
              <input
            ...
            <div>
              <label className="block text-sm font-medium text-ev-muted mb-1.5">Account Number</label>
              <input
        ...
        {/* Existing Requests */}
        <div>
          <h2 className="text-base font-semibold text-ev-text mb-3">Withdrawal History</h2>
          {requests.length > 0 ? (
            <div className="space-y-3">
              {requests.map((req) => (
                <div key={req.id} className="ev-card p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-semibold text-ev-text">Rs. {req.amount.toLocaleString()}</span>
                    {statusBadge(req.status)}
                  </div>
                  <div className="grid grid-cols-2 gap-y-1 text-xs text-ev-muted">
                    <span>Method:</span>
                    <span className="text-ev-text">{req.method}</span>
                    <span>Account:</span>
                    <span className="text-ev-text">{req.account_title}</span>
                    <span>Date:</span>

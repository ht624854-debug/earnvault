...
export default function AdminActivations() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ev-text">Activation Requests</h1>
        <p className="text-ev-muted text-sm mt-1">Review and manage user activation requests</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ev-card-border">
                  ...
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Payment Method</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Sender Name</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Sender Number</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Transaction ID</th>
              ...
              <tbody>
                {requests.map((req) => (
                  <tr key={req.id} className="border-b border-ev-card-border hover:bg-ev-bg transition-colors">
                    ...
                    <td className="py-3 px-4 text-ev-muted">{req.payment_method}</td>
                    <td className="py-3 px-4 text-ev-muted">{req.sender_name}</td>
                    <td className="py-3 px-4 text-ev-muted">{req.sender_number}</td>
                    <td className="py-3 px-4 text-ev-muted font-mono text-xs">{req.transaction_id}</td>
      ...
      <Dialog open={!!rejectDialog} onOpenChange={() => setRejectDialog(null)}>
        <DialogContent className="bg-ev-card border-ev-card-border">
          <DialogHeader>
            <DialogTitle className="text-ev-text">Reject Activation</DialogTitle>
          </DialogHeader>
          <div>
      ...
      <Dialog open={!!proofImage} onOpenChange={() => setProofImage(null)}>
        <DialogContent className="bg-ev-card border-ev-card-border max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-ev-text">Payment Proof</DialogTitle>
          </DialogHeader>
          {proofImage && (

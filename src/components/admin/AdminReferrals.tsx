...
export default function AdminReferrals() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#1E293B]">Referrals</h1>
        <p className="text-[#64748B] text-sm mt-1">View and manage referral records</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#EFF6FF]">
                  <th className="text-left py-3 px-4 text-[#64748B] font-medium">Referrer</th>
                  <th className="text-left py-3 px-4 text-[#64748B] font-medium">Referred User</th>
                  <th className="text-left py-3 px-4 text-[#64748B] font-medium">Status</th>
                  <th className="text-left py-3 px-4 text-[#64748B] font-medium">Reward Status</th>
              ...
              <tbody>
                {referrals.map((ref) => (
                  <tr key={ref.id} className="border-b border-[#EFF6FF] hover:bg-[#F0F7FF] transition-colors">
                    <td className="py-3 px-4 text-[#1E293B]">
                      {ref.referrer?.first_name} {ref.referrer?.last_name}
                      <span className="text-[#64748B] text-xs block">@{ref.referrer?.username}</span>
                    </td>
                    <td className="py-3 px-4 text-[#1E293B]">
                      {ref.referred_user?.first_name} {ref.referred_user?.last_name}
                      <span className="text-[#64748B] text-xs block">@{ref.referred_user?.username}</span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-[#1E293B] font-medium">Rs. {ref.reward_amount}</td>

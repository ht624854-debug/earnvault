...
export default function AdminReferrals() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ev-text">Referrals</h1>
        <p className="text-ev-muted text-sm mt-1">View and manage referral records</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ev-card-border">
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Referrer</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Referred User</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Status</th>
                  <th className="text-left py-3 px-4 text-ev-muted font-medium">Reward Status</th>
              ...
              <tbody>
                {referrals.map((ref) => (
                  <tr key={ref.id} className="border-b border-ev-card-border hover:bg-ev-bg transition-colors">
                    <td className="py-3 px-4 text-ev-text">
                      {ref.referrer?.first_name} {ref.referrer?.last_name}
                      <span className="text-ev-muted text-xs block">@{ref.referrer?.username}</span>
                    </td>
                    <td className="py-3 px-4 text-ev-text">
                      {ref.referred_user?.first_name} {ref.referred_user?.last_name}
                      <span className="text-ev-muted text-xs block">@{ref.referred_user?.username}</span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-ev-text font-medium">Rs. {ref.reward_amount}</td>

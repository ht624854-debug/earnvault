...
export default function AdminTransactions() {
  ...
  const getTypeBadge = (type: string) => {
    const colors: Record<string, string> = {
      ...
      task_reward: 'bg-green-500/10 text-green-400 border-green-500/20',
      referral_reward: 'bg-blue-500/10 text-blue-400 border-blue-500/20',
      balance_adjustment: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20',
      reward_claim: 'bg-pink-500/10 text-pink-400 border-pink-500/20',
    };
    return (
      <Badge className={colors[type] || 'bg-[#1F1F1F] text-[#A3A3A3]'}>
        {type?.replace(/_/g, ' ')}
      </Badge>
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#737373] text-sm mt-1">View all platform transactions</p>
        </div>
        <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1F1F1F]">
                  ...
                  <th className="text-left py-3 px-4 text-[#737373] font-medium">Before</th>
                  <th className="text-left py-3 px-4 text-[#737373] font-medium">After</th>
                ...
                {transactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-[#1F1F1F] hover:bg-[#1A1A1A] transition-colors">
                    <td className="py-3 px-4 text-[#F5F5F5]">
                      ...
                      <span className="text-[#737373] text-xs block">@{tx.user?.username}</span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-[#737373] text-xs">Rs. {tx.balance_before}</td>
                    <td className="py-3 px-4 text-[#737373] text-xs">Rs. {tx.balance_after}</td>
                    <td className="py-3 px-4 text-[#A3A3A3] max-w-[200px] truncate">{tx.description}</td>
